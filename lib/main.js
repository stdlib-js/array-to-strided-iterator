...
function stridedarray2iterator( N, src, stride, offset ) {
	...
	if ( !isNonNegativeInteger( N ) ) {
		throw new TypeError( format( '01h2d', N ) );
	}
	if ( !isCollection( src ) ) {
		throw new TypeError( format( '01h2y', src ) );
	}
	if ( !isInteger( stride ) ) {
		throw new TypeError( format( '01h2z', stride ) );
	}
	if ( !isNonNegativeInteger( offset ) ) {
		throw new TypeError( format( '01h30', offset ) );
	}
	if ( arguments.length > 4 ) {
		fcn = arguments[ 4 ];
		if ( !isFunction( fcn ) ) {
			throw new TypeError( format( '01h31', fcn ) );
		}
		thisArg = arguments[ 5 ];
	}
	idx = offset;
